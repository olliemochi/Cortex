...
# CORTEX MODIFICATION: Import Cortex-specific modules
try:
    from routers import memory, dreaming, cortex_tools, discord, cortex_chat
except ImportError:
    logger.warning('Cortex routers not available')
    memory = None
    dreaming = None
    cortex_tools = None
    discord = None
...
try:
    ...
    from agents.memory_core import MemoryCore
    from agents.dreaming_engine_real import DreamingEngine
except ImportError:
    logger.warning('Cortex agents not available')
    CortexAgent = None
    MemoryCore = None
    DreamingEngine = None

# CORTEX MODIFICATION: Import Cortex environment configuration
try:
    import cortex_env
except ImportError:
    logger.warning('Cortex environment configuration not available')
    cortex_env = None
...
async def lifespan(app: FastAPI):
    ...
    # CORTEX MODIFICATION: Initialize Cortex agent system
    if cortex_env and CortexAgent and MemoryCore and DreamingEngine:
        try:
            log.info('Initializing Cortex agent system...')
            
            # Initialize memory core
            memory_path = getattr(cortex_env, 'MEMORY_PATH', './memory/MEMORY.md')
            app.state.memory_core = MemoryCore(memory_path)
            await app.state.memory_core.initialize()
            log.info(f'Memory core initialized with path: {memory_path}')
            
            # Initialize dreaming engine
            dreams_path = getattr(cortex_env, 'DREAMS_PATH', './memory/DREAMS.md')
            dreaming_enabled = getattr(cortex_env, 'DREAMING_ENABLED', False)
            app.state.dreaming_engine = DreamingEngine(dreams_path, app.state.memory_core)
            await app.state.dreaming_engine.initialize(enabled=dreaming_enabled)
            log.info(f'Dreaming engine initialized with path: {dreams_path}')
            
            # Initialize main Cortex agent
            app.state.cortex_agent = CortexAgent(app)
            await app.state.cortex_agent.initialize()
            log.info('Cortex agent system ready')
        except Exception as e:
            log.error(f'Failed to initialize Cortex agent system: {e}')
    else:
        log.debug('Cortex agent system dependencies not available')
    ...
    # CORTEX MODIFICATION: Shutdown Cortex agent system
    if hasattr(app.state, 'cortex_agent'):
        try:
            await app.state.cortex_agent.shutdown()
            log.info('Cortex agent system shut down')
        except Exception as e:
            log.error(f'Error shutting down Cortex agent: {e}')
...
# CORTEX MODIFICATION: Register Cortex-specific routers
if memory:
    app.include_router(memory.router, tags=['cortex'])
if dreaming:
    app.include_router(dreaming.router, tags=['cortex'])
if cortex_tools:
    app.include_router(cortex_tools.router, tags=['cortex'])
if discord:
    app.include_router(discord.router, tags=['cortex'])
if cortex_chat:
    app.include_router(cortex_chat.router, tags=['cortex'])
